...
Text::Text(std::string newText, std::string newFont, glm::vec2 pos)
{
	...
	scale = 1.0;
	SetPosition(pos);
	glm::mat4 proj = glm::ortho(0.0f, (GLfloat)Camera::GetInstance()->SCR_WIDTH, 0.0f,
		(GLfloat)Camera::GetInstance()->SCR_HEIGHT);
	glUseProgram(Shader::Textprogram);
	glUniformMatrix4fv(glGetUniformLocation(Shader::Textprogram, "proj"), 1, GL_FALSE,
		glm::value_ptr(proj));
...
void Text::Render()
{
	...
	glUseProgram(Shader::Textprogram);
	glUniform3f(glGetUniformLocation(Shader::Textprogram, "textColor"), color.x, color.y, color.z);
